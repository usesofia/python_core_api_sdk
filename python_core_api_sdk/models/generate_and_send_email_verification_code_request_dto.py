...
class GenerateAndSendEmailVerificationCodeRequestDto(BaseModel):
    """
    GenerateAndSendEmailVerificationCodeRequestDto
    """ # noqa: E501
    email: StrictStr
    purpose: StrictStr
    __properties: ClassVar[List[str]] = ["email", "purpose"]

    @field_validator('purpose')
    def purpose_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['SIGN_UP', 'SIGN_IN']):
            raise ValueError("must be one of enum values ('SIGN_UP', 'SIGN_IN')")
        return value
